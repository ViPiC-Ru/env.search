...
d["NET-HOST"]+"' AND timeout = 600"),u=new Enumerator(u);!u.atEnd();){n=u.item();u.moveNext();0==n.statusCode&&(d["TMP-CHECK"]=n.responseTime+" мс");break}}if(!f){var y={};p=w.length;for(e=0;e<p;e++)if(d=w[e],d["TMP-INDEX"]=b.lib.strPad(e+1,(""+p).length,"0","left"),!c.noalign)for(g in d)y[g]=Math.max(d[g].length,y[g]||0)}if(!f)if(p=w.length){q||l.stdOut.writeLine();for(e=0;e<p;e++){d=w[e];d=b.lib.clone(d);if(!c.noalign)for(g in y)d[g]=b.lib.strPad(d[g]||"",y[g]," ",isNaN(parseInt(d[g]))?"right":
"left");if(a=d["TMP-INDEX"])d["TMP-INDEX"]=b.fun.color(c.color?"yellow":null,a);if(a=d["NET-HOST"])d["NET-HOST"]=b.fun.color(c.color?"cyan":null,a);a=b.fun.setDataPattern(c.item,d,!1);l.stdOut.writeLine(a)}b.fun.count(r)&&l.stdOut.writeLine();q=!0}else f=9;if(b.fun.count(r)){if(!(f||"index"in c))try{l.stdOut.write("Введите номер компьютера: "),c.color&&l.stdOut.write(b.fun.color("yellow","",!0)),a=l.stdIn.readLine(),c.color&&l.stdOut.write(b.fun.color("reset","",!0)),a=b.wsh.iconv("cp866","windows-1251",
a),a=isNaN(a)?-1:Number(a)-1,c.index=a,q=!1}catch(z){try{l.stdOut.writeLine()}catch(D){}f=10}f||(n=w[c.index])||(f=11);if(!f)for(g in d=n,w=A.environment(b.val.envType),d)a=d[g],setEnv(w,g,a);if(!f){var C=[];y={};e=0;p=b.fun.count(r);for(g in r){d={};d["TMP-KEY"]=g;d["TMP-VALUE"]=A.expandEnvironmentStrings(r[g]);d["TMP-INDEX"]=b.lib.strPad(e+1,(""+p).length,"0","left");if(!c.noalign)for(g in d)y[g]=Math.max(d[g].length,y[g]||0);C.push(d);e++}}if(!f){p=C.length;q||l.stdOut.writeLine();for(e=0;e<p;e++){d=
C[e];d=b.lib.clone(d);if(!c.noalign)for(g in y)d[g]=b.lib.strPad(d[g]||"",y[g]," ",isNaN(parseInt(d[g]))?"right":"left");if(a=d["TMP-VALUE"])d["TMP-VALUE"]=b.fun.color(c.color?"cyan":null,a);if(a=d["TMP-INDEX"])d["TMP-INDEX"]=b.fun.color(c.color?"yellow":null,a);a=b.fun.setDataPattern(c.unit,d,!1);l.stdOut.writeLine(a)}"action"in c||l.stdOut.writeLine()}if(!(f||"action"in c))try{for(g in e=0,l.stdOut.write("Введите номер действия: "),c.color&&l.stdOut.write(b.fun.color("yellow","",!0)),a=l.stdIn.readLine(),
c.color&&l.stdOut.write(b.fun.color("reset","",!0)),a=b.wsh.iconv("cp866","windows-1251",a),a=isNaN(a)?-1:Number(a)-1,r)e++==a&&(c.action=g)}catch(z){try{l.stdOut.writeLine()}catch(D){}f=12}if(!f)if(c.action in r){a=r[c.action];var E=A.expandEnvironmentStrings(a)}else f=13}if(E){if(!f&&c.service)try{var F=x.connectServer(d["NET-HOST"],"root\\CIMV2")}catch(z){f=14}if(!f&&c.service)try{var B=F.get("Win32_Service.Name='"+c.service+"'");!B.started&&B.startService()&&(f=16)}catch(z){f=15}if(!f)try{A.run(E,
1,!c.nowait)}catch(z){f=17}if(!f&&c.service)try{B=F.get("Win32_Service.Name='"+c.service+"'"),B.started&&B.stopService()&&(f=18)}catch(z){f=15}}if(f&&(a={1:"У входящего параметра отсутствует значение.",2:"У параметра с действием отсутствует значение.",3:"Не удалось инициировать получение поискового запроса.",4:"Указан не поддерживаемый режим.",5:"Обязательные параметры не прошли проверку или отсутствуют.",6:"Задана не допустимая комбинация параметров.",7:"Не удалось получить контейнер для поиска данных.",
